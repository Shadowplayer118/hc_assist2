...
import AddDiseaseModal from "./staff_modals/add_disease_modal";
import EditDiseaseModal from "./staff_modals/edit_disease_modal";

function DiseaseTableStaff() {
  ...
  const fetchDiseaseData = async () => {
    try {
      const response = await axios.post("http://localhost/hc_assist2/src/zbackend_folder/load_disease.php", {
  ...
  const handleDelete = async (recordId) => {
    ...
    try {
      ...
      await axios.post("http://localhost/hc_assist2/src/zbackend_folder/delete_disease.php", {
        record_id: recordId,
        staff_id: staffId
      });
        ...
        <tbody>
          {diseaseRecords.map((record, index) => (
            <tr key={index}>
              ...
              <td>
                <button onClick={() => handleEdit(record.disease_id)}>Edit</button>
                <button onClick={() => handleDelete(record.disease_id)} style={{ marginLeft: "8px" }}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default DiseaseTableStaff;
